...
function selectByCat(connection, cat) {
	connection.query('SELECT * FROM test WHERE cat = ?', [cat], function (error, results, fields) {
	  if (error) throw error;
	  results.forEach(row => console.log(row));
	});
}
...
function insert(connection, cat, price, qty) {
	connection.query('insert into test(cat, price, qty) values (?, ?, ?)', [cat, price, qty], function (error, results, fields) {
	  if (error) throw error;
...
function update(connection, id, qty) {
	connection.query('update test set qty = ? where id=?', [qty, id], function (error, results, fields) {
	  if (error) throw error;
